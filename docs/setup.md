...
If you would like to use MACE force fields in the workflow, follow the instructions to install LAMMPS accordingly, depending on the `MD_type` (`MD_type` = ["lammps+MACE_no_mliap"](https://mace-docs.readthedocs.io/en/latest/guide/lammps.html) or `MD_type` = ["lammps+MACE"](https://mace-docs.readthedocs.io/en/latest/guide/lammps_mliap.html))
