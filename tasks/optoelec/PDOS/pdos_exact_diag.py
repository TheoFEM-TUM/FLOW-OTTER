...
def main(path_configWF: str = "workflow_config.yaml", num_simulations: int = 1, **kwargs) -> Tuple[bool, dict]:
    ...
    print("Start task: pdos_exact_diag", flush=True)
    ...
    for t in chosen_snapshots:
        print(f"Calculate PDOS for snapshot {t} ...", flush=True)
        result = subprocess.run([
            ...
            *julia_flags_optoelec,
            str(dir_code / "optoelec/PDOS/exact_diag_PDOS.jl"),
    ...
    for label in basis_labels:
        dir_pdos_l = dir_pdos / f"{label}/"
        pdos_files = list(dir_pdos_l.glob("PDOS_*.txt"))
