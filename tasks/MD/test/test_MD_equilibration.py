...
def main(path_configWF: str = "workflow_config.yaml", num_simulations: int = 1, **kwargs) -> Tuple[bool, dict]:
    ...
    if equilibrate:
        ...
        plt.xlabel("Step")
        plt.ylabel("Energy")
        plt.plot(step, E, color='blue', label="energy")
        plt.plot(moving_average(step, w), moving_average(E, w), color='orange', label="moving average")
        plt.axhline(y=E_avg_nvt, xmin=x1, xmax=x2, color='red', label="Average")
        plt.axhline(y=E_avg_nvt+E_std_nvt, xmin=x1, xmax=x2, linestyle='--', color='red', label="Std")
        plt.axhline(y=E_avg_nvt-E_std_nvt, xmin=x1, xmax=x2, linestyle='--', color='red')
        plt.axhline(y=E_avg_npt, xmin=x2, xmax=x3, color='red')
        plt.axhline(y=E_avg_npt+E_std_npt, xmin=x2, xmax=x3, linestyle='--', color='red')
        plt.axhline(y=E_avg_npt-E_std_npt, xmin=x2, xmax=x3, linestyle='--', color='red')
        if n1 != 0:
            plt.axvline(x=n1, linestyle='--', color="black")
        plt.axvline(x=n1+n2, linestyle='--', color="black")
