...
if configWF.get("trajectory_file") is None:
    ...
    swg1_MD = StaticWidthGroup({
        t1_checkMD: [], sg1_MD_s: [t1_checkMD]
        }, width=num_simulations)

    cg1_MD = CyclicalGroup({
        t1_checkMD: [], sg1_MD_c: [t1_checkMD]
        }, num_simulations)
...
with PersistentQueue() as pq:
    if configWF.get("trajectory_file") is None:
        # if trajectory file provided, skip MD testing
        pq.submit(Workflow({
            t0_check_simulation: [],
            sg0_check_simulation: [t0_check_simulation],
            t1_testMD_plot: [sg0_check_simulation],
            t2_prep_DFT: [sg0_check_simulation],
